...
module.exports = function(connection) {
  ...
  this.rollupMetrics = function(base_questionnaire_id) {
    return queryService.executeQuery('SELECT questionnaire.id as questionnaire_id, name, sum(value) as value, title, description,base_questionnaire_id,metric_id,questionnaire_metric.id as questionnaire_metric_id FROM questionnaire inner join questionnaire_metric on questionnaire_id = questionnaire.id inner join metric on questionnaire_metric.metric_id = metric.id WHERE status = \'answered\' and base_questionnaire_id = ? group by metric_id', base_questionnaire_id);
  };
  ...
  this.releaseMetricsToDonor = function(base_questionnaire_id) {
    ...
    const findSQL = "SELECT  base_questionnaire_id, metric_id, sum(value) as value, name FROM questionnaire " +
      ...
      "WHERE status = \'answered\' and base_questionnaire_id = ? group by metric_id";
