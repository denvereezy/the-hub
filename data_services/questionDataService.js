...
module.exports = function(connection) {
  ...
  this.showAll = function(questionnaire_id) {
    return queryService.executeQuery('SELECT questionnaire.status, questionnaire_metric.id as questionnaire_metric_id, metric_id, questionnaire_id,base_questionnaire_id, title, description, value from questionnaire_metric inner join questionnaire on questionnaire_metric.questionnaire_id = questionnaire.id inner join metric on metric_id = metric.id where questionnaire_id = ?', questionnaire_id);
  };
